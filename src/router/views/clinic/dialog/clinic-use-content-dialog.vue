<template>
  <v-dialog v-model="isOpen" persistent max-width="600px">
    <v-card>
      <v-card-title>
        <span class="headline">사용 콘텐츠 목록</span>
      </v-card-title>
      <v-card-text>
        <v-container>
          <v-row>
...
<script>
import { getAllClinicUsageCnt } from '@api/clinic.api'
...
export default {
   ...
   */
  created() {
    this.EventBus.$on('contentL_TO_useContentD-1', (condition) => {
      this.isOpen = true
  ...
  methods: {
     ...
     */
    getItem() {
      getAllClinicUsageCnt(this.params).then(
        (response) => {
          this.datas = response
        },
        (error) => {
